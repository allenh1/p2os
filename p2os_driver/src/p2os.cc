...
P2OSNode::P2OSNode(std::shared_ptr<rclcpp::Node> _node)
: node(_node), ptz_(this)
{
  ...
  /* advertise topics */
  pose_pub_ = node->create_publisher<nav_msgs::msg::Odometry>("pose", qos);
  mstate_pub_ = node->create_publisher<p2os_msgs::msg::MotorState>("motor_state", qos);
  grip_state_pub_ = node->create_publisher<p2os_msgs::msg::GripperState>("gripper_state", qos);
  ptz_state_pub_ = node->create_publisher<p2os_msgs::msg::PTZState>("ptz_state", qos);
  sonar_pub_ = node->create_publisher<p2os_msgs::msg::SonarArray>("sonar", qos);
  aio_pub_ = node->create_publisher<p2os_msgs::msg::AIO>("aio", qos);
  dio_pub_ = node->create_publisher<p2os_msgs::msg::DIO>("dio", qos);
  ...
  cmdvel_sub_ = node->create_subscription<geometry_msgs::msg::Twist>(
    "cmd_vel", std::bind(&P2OSNode::cmdvel_cb, this, std::placeholders::_1), qos);
  cmdmstate_sub_ = node->create_subscription<p2os_msgs::MotorState>(
    "cmd_motor_state", std::bind(&P2OSNode::cmdmotor_state, this, std::placeholders::_1), qos);
  gripper_sub_ = node->create_subscription<p2os_msgs::GripperState>(
    "gripper_control", std::bind(&P2OSNode::gripperCallback, this, std::placeholders::_1), qos);
  ptz_cmd_sub_ = node->create_subscription<p2os_msgs::msg::PTZState>(
    "ptz_control", std::bind(&P2OSPtz::callback, &ptz_, std::placeholders::_1), qos);
  veltime = tf2_ros::toMsg(rclcpp::Time::now());
...
void P2OSNode::cmdvel_cb(const std::shared_ptr<geometry_msgs::msg::TwistConstPtr> msg)
{
