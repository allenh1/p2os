...
KineCalc::KineCalc()
  : link1(0.06875f),
    ...
    link4(0.13775f),
    link5(0.11321f) {
    endEffector.p.x = 0.0f;
    endEffector.p.y = 0.0f;
    endEffector.p.z = 0.0f;
    endEffector.n.x = 0.0f;
    endEffector.n.y = 0.0f;
    endEffector.n.z = 0.0f;
    endEffector.o.x = 0.0f;
    endEffector.o.y = -1.0f;
    endEffector.o.z = 1.0f;
    endEffector.a.x = 1.0f;
    endEffector.a.y = 0.0f;
    endEffector.a.z = 0.0f;
...
void KineCalc::SetJointRange (unsigned int joint, double min, double max) {
    jointMin[joint] = fmin(min, max);
    // So that if min > max we reverse them
...
bool KineCalc::CalculateIK (const EndEffector &fromPosition) {
    ...
    do {
        ...
        m1 += 1;
        // So that within the 3 iterations we get m1 = -1, 0, 1
    }
 ...
 * 
 * @param fromPosition  The desired end effector pose
 * @param solutions[][] The four solutions (each with 5 angles) in an array
 * @return Returns the best solution.
 */
int KineCalc::ChooseSolution (const EndEffector &fromPosition, const double solutions[][5]) {
    ...
    for (int ii = 0; ii < 4; ii++) {
        ...
        for (jj = 0; min != errors[jj]; jj++);
        // Find the index at which the min is at
        errors[jj] = 999999;
        order[ii] = jj;
    }
    for (int ii = 0; ii < 4; ii++) {
